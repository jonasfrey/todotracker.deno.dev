...
let f_a_n_u8_o_list = async function(s_id_hashed){
    
        // console.log(o_post_data)
        let a_a_n_u8 = [];
        let n_len_a_n_u8 = 0;
        let a_o_entry = await o_kv.list({prefix: [s_prefix, `o_list`,s_id_hashed]});
        ...
        if(a_n_u8_complete.length == 0){
            let o_v = await o_kv.get([s_prefix, `o_list`,s_id_hashed]);
            if(o_v){
                a_n_u8_complete = o_v.value;
            }
            if(o_v.value == null){
                a_n_u8_complete = new Uint8Array(0);
            }
        }
        return a_n_u8_complete
...
let f_handler = async function(o_request){
    ...
    if(o_request.headers.get('Upgrade') == 'websocket'){
        ...
        let o_ws_client = f_o_ws_client(
            crypto.randomUUID(),
            '', // s_uuid_hashed, will be set later by client
            o_socket
        )
        ...
        o_socket.addEventListener("message", async (o_e) => {
            ...
            // we read the payload. 
            // the first uint8 is always the function id
            // the rest is the data for that function
            ...
            if(o_websocket_function.s_name == 'set_uuid_hashed'){
                ...
                console.log('set uuid hashed to '+s_uuid_hashed);
                o_ws_client.s_uuid_hashed = s_uuid_hashed;
                
                let a_n_u8_o_list = await f_a_n_u8_o_list(s_uuid_hashed);
                console.log({a_n_u8_o_list})
                if(a_n_u8_o_list.length == 0){
                    console.log(`could not find list ${s_uuid_hashed}`)
                    console.log('list does not yet exist');
                    return;
                }
                let a_n_u8_payload = new Uint8Array(1 + a_n_u8_o_list.length);
                a_n_u8_payload[0] = a_o_websocket_function.find(o=>o.s_name=='payload_is_a_n_u8_encrypted_o_list').n_id;
                a_n_u8_payload.set(a_n_u8_o_list, 1);
            ...
            if(o_websocket_function.s_name == 'read_o_list'){
            ...
            // console.log({o_websocket_function, o_ws_client });
